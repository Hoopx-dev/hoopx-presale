...
function TransactionItemWithGrouping({
  ...
  const showDateHeader =
    index === 0 ||
    (prevTransaction &&
      !isSameDate(transaction.timestamp, prevTransaction.timestamp));
...
export default function PortfolioPage() {
  ...
  const successfulOrders = useMemo(() => {
    return (
      purchaseSession?.orderVoList?.filter(
  ...
  const hasPurchasedCurrentActivity = useMemo(() => {
    ...
    return successfulOrders.some(
      (order) => order.activityId === purchaseDetails.activityId
    );
          ...
          {activeTab === "transactions" && (
            <div className='space-y-4'>
              ...
              {successfulOrders.map((order, index) => {
                ...
                const prevOrder =
                  index > 0 ? successfulOrders[index - 1] : null;
