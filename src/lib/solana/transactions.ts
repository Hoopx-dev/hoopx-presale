...
export async function fetchTransactionBySignature(
  ...
  try {
    ...
    console.log(
      "[Transaction Query] Transaction fetched:",
      tx ? "Found" : "Not found"
    );
    ...
    if (!transferInfo) {
      console.log(
        "[Transaction Query] Could not parse transfer info from transaction"
      );
      return null;
    }
 ...
 */
function parseTokenTransfer(tx: ParsedTransactionWithMeta): {
  amount: number;
  from: string;
  to: string;
  tokenSymbol: string;
  ...
  try {
    ...
    for (let i = 0; i < instructions.length; i++) {
      ...
      console.log(
        `[Parse Transfer] Instruction ${i}:`,
        "parsed" in ix ? "Has parsed" : "No parsed"
      );
      ...
      if ("parsed" in ix && ix.parsed && typeof ix.parsed === "object") {
        ...
        console.log(
          `[Parse Transfer] Instruction ${i} info:`,
        ...
        if (parsed.type === "transfer" || parsed.type === "transferChecked") {
          ...
          if (!source || !destination) {
            console.log(
              "[Parse Transfer] Missing source or destination, skipping..."
            );
            continue;
          }
          ...
          if (tx.meta?.preTokenBalances && tx.meta?.postTokenBalances) {
            ...
            if (destBalance && destBalance.owner) {
              ...
              console.log(
                "[Parse Transfer] Found destination owner:",
                toAddress
              );
            }
          }
          ...
          console.log(
            "[Parse Transfer] Token mint from instruction:",
            tokenMint
          );
          ...
          if (!tokenMint && tx.meta?.postTokenBalances) {
            ...
            if (destBalance) {
              ...
              console.log(
                "[Parse Transfer] Token mint from balance:",
                tokenMint
              );
            }
          }
